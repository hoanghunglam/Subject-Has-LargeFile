...
(function (app) {
    'use strict';
    app.controller('CanhanAddController', CanhanAddController);
    // $state đối tượng  của ui-router để điều hướng 
    CanhanAddController.$inject = ['$scope', 'apiService', 'notificationService', '$state', 'authData'];

    function CanhanAddController($scope, apiService, notificationService, $state, authData) {
        $scope.participant = {
            $class: 'org.dispatch.CaNhan',
            donVi: 'HV_KTQS',
            khoa: 'KH_CNTT',
            participantId: '',
            hoTen: '',
        ...
        function AddParticipant() {
            apiService.postBlock('http://' + hostLam + ':3000/api/org.dispatch.CaNhan', $scope.participant, {
                headers: {
