(function (app) {
    app.controller('CanhanListController', CanhanListController);
    CanhanListController.$inject = ['$scope', 'apiService', 'notificationService', '$ngBootbox', 'authData'];

    function CanhanListController($scope, apiService, notificationService, $ngBootbox, authData) {
        ;
        ...
        function getParticipantList() {
            apiService.getBlock('http://' + hostLam + ':3000/api/queries/AllCaNhan', {
                headers: {
                ...
                for (var i = 0; i < $scope.getParticipant.length; i++) {
                    ...
                    switch ($scope.getParticipant[i].khoa) {
                        case "KH_CNTT":
                            $scope.getParticipant[i].khoa = "Khoa công nghệ thông tin";
                            break;
                        case "KH_XD":
                            $scope.getParticipant[i].khoa = "Khoa xây dựng";
                            break;
                        case "KH_TOAN":
                            $scope.getParticipant[i].khoa = "Khoa toán";
                            break;
                        case "KH_CDT":
                            $scope.getParticipant[i].khoa = "Khoa cơ điện tử";
                            break;
                        case "KH_CK":
                            $scope.getParticipant[i].khoa = "Khoa cơ khí";
                            break;
                        case "KH_HHVT":
                            $scope.getParticipant[i].khoa = "Khoa hàng không vũ trụ";
                            break;
                    }
        ...
        function deleteParticipant(ID) {
            $ngBootbox.confirm("Bạn chắc chắn muốn xóa!").then(function () {
                apiService.delBlock('http://' + hostLam + ':3000/api/org.dispatch.CaNhan/' + ID, {
                    headers: {
                        'X-Access-Token': access_tokens
                    }
                }, function () {
