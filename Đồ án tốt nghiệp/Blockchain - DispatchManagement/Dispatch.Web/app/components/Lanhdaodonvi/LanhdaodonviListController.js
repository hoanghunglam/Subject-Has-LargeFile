(function (app) {
    app.controller('LanhdaodonviListController', LanhdaodonviListController);
    LanhdaodonviListController.$inject = ['$scope', 'apiService', 'notificationService', '$ngBootbox', 'authData'];

    function LanhdaodonviListController($scope, apiService, notificationService, $ngBootbox, authData) {
        ;
        ...
        function getParticipantList() {
            apiService.getBlock('http://' + hostLam + ':3000/api/queries/AllLanhDaoDV', {
                headers: {
        ...
        function deleteParticipant(ID) {
            $ngBootbox.confirm("Bạn chắc chắn muốn xóa!").then(function () {
                apiService.delBlock('http://' + hostLam + ':3000/api/org.dispatch.LanhDaoDV/' + ID, {
                    headers: {
                        'X-Access-Token': access_tokens
                    }
                }, function () {
