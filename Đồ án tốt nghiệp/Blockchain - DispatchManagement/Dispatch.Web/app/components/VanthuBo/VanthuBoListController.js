...
(function (app) {
    app.controller('VanthuBoListController', VanthuBoListController);
    VanthuBoListController.$inject = ['$scope', 'apiService', 'notificationService', '$ngBootbox', 'authData'];

    function VanthuBoListController($scope, apiService, notificationService, $ngBootbox, authData) {
        ;
        ...
        function getParticipantList() {
            apiService.getBlock('http://' + hostLam + ':3000/api/queries/AllVanThuCuaBo', {
                headers: {
        ...
        function deleteParticipant(ID) {
            $ngBootbox.confirm("Bạn chắc chắn muốn xóa!").then(function () {
                apiService.del('http://' + hostLam + ':3000/api/org.dispatch.VanThuCuaBo/' + ID, {
                    headers: {
                        'X-Access-Token': access_tokens
                    }
                }, function () {
